...
## Rule: commits only on explicit approval

Агент не делает `git commit` / `git push` / `git revert` / `git reset` без
явного согласия мейнтейнера на конкретное действие. «Подготовь коммит»
означает stage + показ diff, но не коммит. Самовольный пуш запрещён даже
при «очевидной» готовности — спросить, дождаться подтверждения.
